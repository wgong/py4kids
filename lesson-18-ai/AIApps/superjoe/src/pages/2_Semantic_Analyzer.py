...
def show_semantic_analyzer():
    ...
    relationships between words and shows their related concepts and meanings.
    
    Enter any two words to see:
    - How similar they are in meaning
    - What other concepts they relate to
    - Their semantic field analysis
    """)
    ...
    with col1:
        text1 = st.text_area("Enter first word/concept", height=100, key='text1',
                            placeholder="Enter a word or concept (e.g., dog, human, book)")
        
    with col2:
        text2 = st.text_area("Enter second word/concept", height=100, key='text2',
                            placeholder="Enter another word or concept to compare")
        ...
        if text1 and text2:
            with st.spinner("Analyzing semantic relationships..."):
                try:
                    ...
                    # Get semantic fields
                    field1 = get_semantic_field(text1, 'English', model)
                    field2 = get_semantic_field(text2, 'English', model)
                    ...
                    col1, col2 = st.columns(2)
                    with col1:
                        st.write("**First word semantic field:**")
                        for word, sim in field1[:5]:
                            st.write(f"- {word}: {sim:.2f} {get_similarity_display(sim)}")
                    with col2:
                        st.write("**Second word semantic field:**")
                        for word, sim in field2[:5]:
                            st.write(f"- {word}: {sim:.2f} {get_similarity_display(sim)}")

                    # Visualization
                    st.subheader("Comparative Analysis")
                    fig = plot_semantic_comparison(field1, field2, "First Word", "Second Word")
