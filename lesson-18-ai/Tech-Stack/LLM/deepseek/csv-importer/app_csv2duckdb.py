...
def add_download_buttons(dataset_name):
    """Add download buttons for DDL and DuckDB database."""
    try:
        ...
        # Download DuckDB DB
        db_path = f"db/{dataset_name}/{dataset_name}.duckdb"
        if os.path.exists(db_path):
            ...
            col2.download_button(
                "📥 Download DuckDB DB",
                db_content,
                file_name=f"{dataset_name}.duckdb",
                mime="application/octet-stream"
            )
    except Exception as e:
        st.error(f"Error setting up download buttons: {str(e)}")

def create_duckdb_ddl(df, table_name, file_name, column_mapping):
    """Generate DuckDB DDL with comments for original filename and column names."""
    try:
        type_map = {
            'int64': 'INTEGER',
            'float64': 'DOUBLE',
            'object': 'VARCHAR',
            'datetime64[ns]': 'VARCHAR',  # Store timestamps as VARCHAR
            'bool': 'BOOLEAN'
        }
        ...
        for orig_col, snake_col in column_mapping.items():
            sql_type = type_map.get(str(df[orig_col].dtype), 'VARCHAR')
...
def convert_timestamps_to_string(df):
    """Convert timestamp columns to strings (VARCHAR)."""
    ...
    if st.session_state.dataframes:
        ...
        for file_name, df in st.session_state.dataframes.items():
            ...
            new_table_name = st.text_input(
                f"Table name:",
                value=st.session_state.table_names[file.name],
            ...
            try:
                ...
                for file_name, df in st.session_state.dataframes.items():
                    ...
                    try:
                        ...
                        df_renamed = convert_timestamps_to_string(df_renamed)
                        
                        # Load data into DuckDB
                        conn.register('temp_df', df_renamed)
