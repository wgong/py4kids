...
pipeline. The human writes `@spec` once; the system generates, tests, and
verifies the implementation autonomously — and checks its own work.
...
clarification of the right methodology for applying NDD closure to real
systems:
...
The `@item` gap is a perfect example: the user-guide observation ("collection
iteration doesn't work") led directly to `_exec_while` without requiring a
full read of all 1,400 lines of the executor.

### 6.5 Source of Truth Clarification

This exercise also produced an important principle about living software
projects:
...
All three artefacts are in `SPL20/docs/NDD-closure/`:
