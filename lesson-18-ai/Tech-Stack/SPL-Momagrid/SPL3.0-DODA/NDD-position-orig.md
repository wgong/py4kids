...
**Companion document:** `NDD-closure.md` (formal definition of NDD closure property)
...
We propose the term **agentic integrity** to name this property precisely: the degree to which an agent — human or artificial — consistently satisfies `J(stated_intent, extract_intent(behavior)) = CLOSED` over time.
...
### 4.3 Human-in-the-loop as the boundary condition

A deliberate design decision in the SPL v3.0 reference implementation: `[DIVERGED]` triggers a **hard stop**, not an automatic refinement loop.
...
```User
When Gemini does the deep literature search, "agentic integrity" and "closure-based AI safety" are worth searching as exact phrases
```