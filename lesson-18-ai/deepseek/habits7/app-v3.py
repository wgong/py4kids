...
def main():
    ...
    if choice == "Home":
        ...
        with row1_col1:
            ...
            grid_resp_2 = _display_df_grid(df_2, key_name="df_2", page_size=5, grid_height=250)
            # st.dataframe(df_2)
            st.write("II: Un-Important/Urgent")

        with row1_col2:
            df_1 = filtered_df[(filtered_df['Is Important'] == 'Y') & (filtered_df['Is Urgent'] == 'Y')][selected_columns]
            grid_resp_1 = _display_df_grid(df_1, key_name="df_1", page_size=5, grid_height=250)
            # st.dataframe(df_1)
            st.write("I: Important/Urgent")

        row2_col1, row2_col2 = st.columns(2)
        with row2_col1:
            df_3 = filtered_df[(filtered_df['Is Important'] == 'N') & (filtered_df['Is Urgent'] == 'N')][selected_columns]
            grid_resp_3 = _display_df_grid(df_3, key_name="df_3", page_size=5, grid_height=250)
            # st.dataframe(df_3)
            st.write("III: Un-Important/Un-Urgent")

        with row2_col2:
            df_4 = filtered_df[(filtered_df['Is Important'] == 'Y') & (filtered_df['Is Urgent'] == 'N')][selected_columns]
            grid_resp_4 = _display_df_grid(df_4, key_name="df_4", page_size=5, grid_height=250)
            # st.dataframe(df_4)
            st.write("IV: Important/Un-Urgent")
