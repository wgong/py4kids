...
**The idea:** Momagrid currently treats volunteer compute as a gift (school gaming PCs during idle
hours). The Hub-to-Hub federation, cryptographic agent identity, and tier-aware dispatch already
contain the building blocks of a **compute marketplace**.

**The mutual-aid model:** A student contributes 4 hours of GPU time to the school grid and earns
credits. Those credits are redeemable for inference time on a higher-tier node (a donated cloud
GPU, a university cluster). The Hub tracks contributions via the existing rewards system
(`mg rewards`). No cryptocurrency needed — just a ledger of compute-hours, auditable because
every task is logged.

**The shift:** From charity ("donated hardware") to mutual aid ("I contribute what I have, I draw
what I need"). This creates a natural incentive for the grid to grow organically — participation
is rewarded by access, not by external subsidy.

**Research needed:** Credit ledger design (compute-hour unit, exchange rate between node tiers);
abuse-resistance (Sybil attacks, fake contribution claims); integration with Hub's existing task
log as the audit source of truth.
...
| Momagrid Hub federation + task log | 5E (compute marketplace), 5H (audit trail) |
