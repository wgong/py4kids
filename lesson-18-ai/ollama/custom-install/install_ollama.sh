...
if check_gpu lspci amdgpu || check_gpu lshw amdgpu; then
    ...
    $SUDO rm -rf $OLLAMA_HOME/lib
    $SUDO chmod o+x $OLLAMA_HOME
    $SUDO install -o ollama -g ollama -m 755 -d $OLLAMA_HOME/lib/rocm
    curl --fail --show-error --location --progress-bar "https://ollama.com/download/ollama-linux-amd64-rocm.tgz${VER_PARAM}" \
        | $SUDO tar zx --owner ollama --group ollama -C $OLLAMA_HOME/lib/rocm .
    install_success
    status "AMD GPU ready."
    exit 0
fi
